...
use crate::{
    analyzer::{Entity, Program, Scope},
    shared::{BinaryOp, Error, Value},
};
...
fn interpret_entity<'src>(
    ...
    Ok(match entity {
        ...
        Entity::Function(id) => {
            println!(
                "Found a function with id {:#?} and name {:#?}",
                id,
                program.get_function(id).map(|x| x.name)
            );
            Value::Null
        }
        Entity::Call(subject, args) => match &**subject {
            Entity::Local(id) => {
