...
A moment in time — epoch milliseconds in an `i64` under the hood.
...
impl Duration {
	// constructors
	fun millis(count: i64): Duration
	fun seconds(count: i64): Duration
	fun minutes(count: i64): Duration
	fun hours(count: i64): Duration
	fun days(count: i64): Duration
	// truncating accessors
	fun as_seconds(self): i64
	fun as_minutes(self): i64
	fun as_hours(self): i64
	fun as_days(self): i64
...
fun main() {
	let started = now();
	let deadline = started + Duration::hours(2i64);
...
- Duration constructors take `i64` — remember the suffix on computed
  literals (`Duration::millis(500i64 * factor)`); see
...
- Wire format: an `Instant` serializes as its `i64` millis — exact for any
  realistic date (i64 rides the wire as a float's 53 bits, safe past year
  200,000).
